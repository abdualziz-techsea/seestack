package com.seestack.modules.cron.service;

import com.seestack.modules.cron.dto.CronPingResponse;
...
public class CronPingQueryService {
    ...
    public @Nullable CronPingResponse findLastPing(UUID monitorId) {
        String sql = "SELECT id, status, duration_ms, message, timestamp FROM seestack.cron_pings WHERE monitor_id = ? ORDER BY timestamp DESC LIMIT 1";
    ...
    public boolean hasMissedPingAfter(UUID monitorId, Instant after) {
        String sql = "SELECT count() FROM seestack.cron_pings WHERE monitor_id = ? AND status = 'missed' AND timestamp >= toDateTime64(?, 3)";
        try (Connection conn = clickHouseDataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, monitorId.toString());
    ...
    public double avgDurationLastN(UUID monitorId, int n) {
        String sql = "SELECT avg(duration_ms) as avg_dur FROM (SELECT duration_ms FROM seestack.cron_pings WHERE monitor_id = ? AND status = 'success' AND duration_ms > 0 ORDER BY timestamp DESC LIMIT ?)";
        try (Connection conn = clickHouseDataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, monitorId.toString());
            ps.setInt(2, n);
    ...
    public HistoryResult getHistory(UUID monitorId, @Nullable String status,
        ...
        try (Connection conn = clickHouseDataSource.getConnection()) {
            long total;
            try (PreparedStatement ps = conn.prepareStatement("SELECT count() FROM seestack.cron_pings WHERE " + where)) {
                for (int i = 0; i < params.size(); i++) ps.setObject(i + 1, params.get(i));
                try (ResultSet rs = ps.executeQuery()) { rs.next(); total = rs.getLong(1); }
            }

            List<CronPingResponse> items = new ArrayList<>();
            String query = "SELECT id, status, duration_ms, message, timestamp FROM seestack.cron_pings WHERE " + where + " ORDER BY timestamp DESC LIMIT ? OFFSET ?";
