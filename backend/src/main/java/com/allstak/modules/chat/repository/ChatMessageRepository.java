...
public interface ChatMessageRepository extends JpaRepository<ChatMessageEntity, UUID> {
    ...
    @Query("SELECT m FROM ChatMessageEntity m JOIN com.allstak.modules.chat.entity.ChatChannelEntity c ON c.id = m.channelId WHERE c.orgId = ?1 AND m.deleted = false AND LOWER(m.content) LIKE LOWER(CONCAT('%', ?2, '%')) ORDER BY m.createdAt DESC")
