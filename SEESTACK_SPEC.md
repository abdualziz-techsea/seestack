# SeeStack — Project Specification
...
SeeStack is the **single home base** for engineering teams. Developers should never need to leave SeeStack — errors, logs, monitors, servers, and team communication all live in one place.
...
| Payment flow | SeeStack creates invoice -> redirects user to Moyasar checkout -> webhook confirms |
| Webhook | Moyasar POSTs to SeeStack callback_url on payment |
...
| Tool | SeeStack Pro $49 | Separate tools |
|---|---|---|
...
### SDK / Ingestion API (App → SeeStack)

Authenticated via `API Key` in header: `X-SeeStack-Key: <key>`
...
```
Organization: https://github.com/SeeStack

Repositories:
├── SeeStack/seestack          ← main monorepo (backend + frontend + infra)
├── SeeStack/seestack-js       ← JavaScript/TypeScript SDK
├── SeeStack/seestack-java     ← Java SDK
└── SeeStack/seestack-python   ← Python SDK
```
...
├── Backend: Kafka topic setup (seestack.errors)
...
├── Backend: Kafka topic setup (seestack.http_requests)
...
| Feature | What it means for SeeStack |
|---|---|
...
Auto-configures metrics + traces export over OTLP with zero extra setup. Relevant for SeeStack's own internal observability.
...
**Path:** `C:\Users\iRovler\Desktop\MyProjects\seestack-test-projects`

Rules:
- Must be used for real-world validation of every observability feature
- Must send real logs, errors, and events to SeeStack via `seestack-js` SDK
...
Using **seestack-js** (`C:\Users\iRovler\Desktop\MyProjects\seestack-js`):
...
```
seestack-js SDK  →  POST /ingest/v1/logs (X-SeeStack-Key header)
                →  Kafka topic: seestack.logs
                →  LogKafkaConsumer → LogClickHouseWriter
                →  ClickHouse: seestack.logs table
...
- [ ] Tested with real incoming logs sent via `seestack-js` SDK over time (not a single batch)
...
- [ ] Test script: `seestack-test-projects/test-live-tail.mjs` sends 12 logs every 3s across 3 services
